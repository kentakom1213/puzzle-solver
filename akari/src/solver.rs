...
use crate::field::{Field, Solution};
...
pub mod solvers {
    use crate::{
        ...
        solver::{MISMATCH_AKARI, OVERLAP_AKARI, Solver, UNLIT_CELL},
    ...
    impl BackTrack {
        ...
        fn rec(field: &Field, sol: &mut Solution) -> Option<Solution> {
            todo!()
        }
    }
    ...
    mod test_backtrack {
        use crate::{
            field::{Field, Solution, State},
            solver::{MISMATCH_AKARI, OVERLAP_AKARI, UNLIT_CELL, solvers::BackTrack},
        };
        ...
        fn test_check() {
            ...
            let field = Field {
                ...
                w: 3,
                field: vec![
                    vec![State::Adj2, State::Empty, State::Adj1],
                    vec![State::Empty, State::Empty, State::Empty],
                    vec![State::Empty, State::Empty, State::Adj0],
                ],
            };
            let sol = Solution {
                field: vec![
